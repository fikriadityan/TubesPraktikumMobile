...
class MobilRepository @Inject constructor(
    private val api: MobilApi, private val dao: MobilDao
) : Repository {
    suspend fun loadItems(
        onSuccess: (List<Mobil>) -> Unit, onError: (List<Mobil>, String) -> Unit
    ) {
...
// e.g. network connection error.
    ...
    suspend fun insert(
        ...
        val item = Mobil(id, merk, model, bahanBakar, dijual, deskripsi)
        dao.insertAll(item)
        api.insert(item)
// handle the case when the API request gets a success response.
            .suspendOnSuccess {
